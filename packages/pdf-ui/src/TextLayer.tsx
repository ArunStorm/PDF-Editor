...
export default function TextLayer({
  ...
  useEffect(() => {
    ...
    // PDF.js text extraction can complete after the first paint on slower
    // browsers/CI runners. Retry briefly so Edit Text never becomes a dead tool.
      ...
      while (!cancelled && attempts < 10) {
        ...
        try {
          ...
          const textItems = content.items
            ...
            .map((item: any) => ({ ...item, style: content.styles?.[item.fontName] })) as TextItem[];

          if (textItems.length > 0 || attempts === 10) {
            if (!cancelled) setItems(textItems);
            return;
          }
        } catch (error) {
          if (attempts === 10) console.error('[PDF Editor] text layer failed:', error);
        }
  ...
  return (
    <div
      ...
      data-text-item-count={items.length}
