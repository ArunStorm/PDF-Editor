...
type TextItem = { str: string; transform: number[]; width: number; height?: number; fontName?: string; style?: TextStyle; hasEOL?: boolean };
type OcrWord = { text: string; x: number; y: number; width: number; height: number; confidence?: number };
type MeasuredItem = TextItem & { x: number; right: number; baseline: number; fontSize: number; top: number; bottom: number; family: string; fontWeight: string; fontStyle: string };
type TextLine = { items: MeasuredItem[]; x: number; right: number; baseline: number; top: number; bottom: number; fontSize: number; family: string; fontWeight: string; fontStyle: string; bullet: boolean; firstItemBullet: boolean };
type OcrLine = { words: OcrWord[]; x: number; right: number; y: number; bottom: number; fontSize: number; bullet: boolean };
...
type TextBlock = { items: Array<MeasuredItem | OcrWord>; lines: Array<TextLine | OcrLine>; text: string; x: number; y: number; width: number; height: number; fontSize: number; fontName?: string; fontFamily: string; fontWeight: string; fontStyle: string; lineHeight: number; bullet: boolean };
...
function typography(fontName = '', pdfFamily = '') {
  ...
  const family = /times|roman|serif/.test(name) && !/sans|arial|helvetica/.test(name) ? 'Times New Roman, serif' : /courier|mono/.test(name) ? 'Courier New, monospace' : 'Arial, Helvetica, sans-serif';
...
function lineHeightFor(lines: TextLine[], fontSize: number) { const deltas = lines.slice(1).map((line, index) => Math.abs(line.baseline - lines[index].baseline)).filter((value) => value > fontSize * 0.55 && value < fontSize * 2.5); const average = deltas.length ? deltas.reduce((sum, value) => sum + value, 0) / deltas.length : fontSize * 1.2; return Math.max(fontSize * 1.08, average); }
...
export function groupTextItems(items: TextItem[], viewport: pdfjsLib.PageViewport): TextBlock[] {
  ...
  const measured: MeasuredItem[] = items.map((item) => {
    ...
    const ascent = Math.max(0.55, item.style?.ascent ?? 0.9); const descent = Math.min(-0.05, item.style?.descent ?? -0.2);
    const axisScale = Math.max(0.001, Math.hypot(transform[0], transform[1])); const advanceX = item.width * pageScale * (transform[0] / axisScale);
    const x1 = transform[4]; const x2 = x1 + advanceX;
  ...
  for (const item of measured) {
    ...
    const candidates = lines.filter((line) => {
      ...
      const crossesColumn = item.x > pageWidth * 0.52 && line.right < item.x && gap > pageWidth * 0.14;
      return gap <= Math.max(100, item.fontSize * 8) && !crossesColumn;
    });
  ...
  for (const line of lines) {
    let best: { block: TextBlock; score: number } | undefined;
    if (!line.bullet) {
      for (const block of blocks) {
        const previous = block.lines[block.lines.length - 1] as TextLine;
        if (!previous || line.baseline <= previous.baseline || !sameTypography(previous.items[0], line.items[0])) continue;
        const verticalGap = line.top - previous.bottom;
        const startDelta = Math.abs(line.x - previous.x);
        const horizontalOverlap = line.x < previous.right && previous.x < line.right;
        const sameColumn = startDelta <= Math.max(36, line.fontSize * 3.5) || horizontalOverlap;
        const lineSpacing = Math.abs(line.baseline - previous.baseline);
        const maxLineSpacing = Math.max(18, previous.fontSize * 1.75, block.lineHeight * 1.5);
        const paragraphGap = Math.max(0, verticalGap);
        if (lineSpacing > maxLineSpacing || paragraphGap > Math.max(14, previous.fontSize * 1.25) || !sameColumn) continue;
        const score = paragraphGap * 4 + startDelta;
...
function groupOcrWords(words: OcrWord[]): TextBlock[] {
  ...
  for (const word of sorted) { const center = word.y + word.height / 2; const candidate = lines.find((line) => Math.abs((line.y + line.bottom) / 2 - center) <= Math.max(4, word.height * 0.55) && word.x >= line.x - Math.max(4, word.height * 0.25) && word.x - line.right <= Math.max(24, word.height * 2.5)); if (!candidate) lines.push({ words: [word], x: word.x, right: word.x + word.width, y: word.y, bottom: word.y + word.height, fontSize: Math.max(5, word.height * 0.8), bullet: isBullet(word.text) }); else { candidate.words.push(word); candidate.right = Math.max(candidate.right, word.x + word.width); candidate.x = Math.min(candidate.x, word.x); candidate.y = Math.min(candidate.y, word.y); candidate.bottom = Math.max(candidate.bottom, word.y + word.height); candidate.fontSize = Math.max(candidate.fontSize, word.height * 0.8); candidate.bullet ||= isBullet(word.text); } }
...
export default function TextLayer({ page, viewport, enabled, onEdit }: { page: pdfjsLib.PDFPageProxy; viewport: pdfjsLib.PageViewport; enabled: boolean; onEdit: (selection: TextSelection, replacement: string) => void }) {
  ...
  const blocks = useMemo(() => source === 'ocr' ? groupOcrWords(ocrWords) : groupTextItems(items, viewport), [items, ocrWords, source, viewport]);
  useEffect(() => { let cancelled = false; setEditingIndex(null); setItems([]); setOcrWords([]); setSource('pdf'); const loadText = async () => { for (let attempt = 0; attempt < 8 && !cancelled; attempt += 1) { try { const streamed: any[] = []; const reader = page.streamTextContent({ includeMarkedContent: false }).getReader(); while (!cancelled) { const { value, done } = await reader.read(); if (done) break; if (value?.items) streamed.push(...value.items); } let textItems = streamed.filter((item: any) => typeof item.str === 'string' && item.str.trim()).map((item: any) => ({ ...item })) as TextItem[]; if (textItems.length === 0) { const content = await page.getTextContent({ includeMarkedContent: false }); textItems = content.items.filter((item: any) => typeof item.str === 'string' && item.str.trim()).map((item: any) => ({ ...item })) as TextItem[]; } if (textItems.length > 0) { const content = await page.getTextContent({ includeMarkedContent: false }); const styles = content.styles ?? {}; if (!cancelled) setItems(textItems.map((item) => ({ ...item, style: styles[item.fontName ?? ''] }))); return; } const canvas = rootRef.current?.parentElement?.querySelector('canvas') ?? null; if (canvas && !cancelled) { setSource('ocr'); const worker = await createWorker('eng'); try { const result = await worker.recognize(canvas); const data: any = result.data; const dpr = Math.max(1, canvas.width / Math.max(1, canvas.clientWidth)); const words: OcrWord[] = (data.words ?? []).map((word: any) => ({ text: String(word.text ?? '').trim(), x: Number(word.bbox?.x0 ?? 0) / dpr, y: Number(word.bbox?.y0 ?? 0) / dpr, width: Math.max(0, Number(word.bbox?.x1 ?? 0) - Number(word.bbox?.x0 ?? 0)) / dpr, height: Math.max(0, Number(word.bbox?.y1 ?? 0) - Number(word.bbox?.y0 ?? 0)) / dpr, confidence: Number(word.confidence ?? 0) })).filter((word: OcrWord) => word.text && word.width > 1 && word.height > 1); if (!cancelled) setOcrWords(words); } finally { await worker.terminate(); } return; } } catch (error) { if (attempt === 7) console.error('[PDF Editor] text/OCR layer failed:', error); } await new Promise((resolve) => window.setTimeout(resolve, 150)); } if (!cancelled) { setItems([]); setOcrWords([]); } }; void loadText(); return () => { cancelled = true; }; }, [page, enabled]);
