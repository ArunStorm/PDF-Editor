...
/** Reconstruct PDF.js fragments into visual lines and then Acrobat-like logical text blocks. */
export function groupTextItems(items: TextItem[], viewport: pdfjsLib.PageViewport): TextBlock[] {
  ...
  for (const item of measured) {
    const tolerance = Math.max(2.5, item.fontSize * 0.45);
    const candidates = lines.filter((line) => Math.abs(line.baseline - item.baseline) <= tolerance && sameTypography(line.items[0], item) && item.x >= line.x - Math.max(3, item.fontSize * 0.35));
    ...
    const columnGap = Math.max(42, item.fontSize * 4.5);
    const rightColumn = item.x > pageWidth * 0.52 && line.right < item.x && gap > pageWidth * 0.18;
    if (gap > columnGap || rightColumn || line.items[line.items.length - 1].hasEOL) {
      const bullet = isBullet(item.str);
      lines.push({ items: [item], x: item.x, right: item.right, baseline: item.baseline, top: item.top, bottom: item.bottom, fontSize: item.fontSize, family: item.family, fontWeight: item.fontWeight, fontStyle: item.fontStyle, bullet, firstItemBullet: bullet });
      continue;
    }
  ...
  for (const line of lines) {
    ...
    const previousLine = previous?.lines[previous.lines.length - 1] as TextLine | undefined;
    if (!previous || !previousLine || line.baseline <= previousLine.baseline || line.bullet) {
      blocks.push(makeBlock(line));
      continue;
    }
    const verticalGap = line.top - previousLine.bottom;
    const sameStyle = sameTypography(previousLine.items[0], line.items[0]);
    const lineHeight = Math.max(previousLine.bottom - previousLine.top, line.bottom - line.top, line.fontSize * 1.05);
    ...
    const compatibleIndent = startDelta <= Math.max(24, line.fontSize * 2.2);
    const continuationOfBullet = previous.bullet && !line.bullet && line.x >= previousLine.x;
    const paragraphGap = Math.max(7, lineHeight * 0.95);
    const sameParagraph = sameStyle && verticalGap >= -line.fontSize * 0.45 && verticalGap <= paragraphGap && (horizontalOverlap || compatibleIndent || continuationOfBullet);
    if (!sameParagraph) {
      blocks.push(makeBlock(line));
      continue;
    }
...
function groupOcrWords(words: OcrWord[]): TextBlock[] {
  ...
  for (const line of lines) {
    const previous = blocks[blocks.length - 1];
    const previousLine = previous?.lines[previous.lines.length - 1] as OcrLine | undefined;
    ...
    const gap = line.y - previousLine.bottom;
    const startDelta = Math.abs(line.x - previousLine.x);
    if (gap < -line.fontSize * 0.45 || gap > Math.max(8, line.fontSize * 1.0) || startDelta > Math.max(24, line.fontSize * 2.2)) { blocks.push({ items: [...line.words], lines: [line], text: textOf(line), x: line.x, y: line.y, width: line.right - line.x, height: line.bottom - line.y, fontSize: line.fontSize, fontFamily: 'Arial, Helvetica, sans-serif', fontWeight: '400', fontStyle: 'normal', lineHeight: Math.max(8, line.bottom - line.y), bullet: line.bullet }); continue; }
...
export default function TextLayer({ page, viewport, enabled, onEdit }: { page: pdfjsLib.PDFPageProxy; viewport: pdfjsLib.PageViewport; enabled: boolean; onEdit: (selection: TextSelection, replacement: string) => void }) {
  ...
  const [source, setSource] = useState<'pdf' | 'ocr'>('pdf');
  ...
  useEffect(() => {
    let cancelled = false;
    setEditingIndex(null); setItems([]); setOcrWords([]); setSource('pdf');
    const loadText = async () => {
      for (let attempt = 0; attempt < 8 && !cancelled; attempt += 1) {
        try {
          const streamed: any[] = [];
          const reader = page.streamTextContent({ includeMarkedContent: false }).getReader();
          while (!cancelled) { const { value, done } = await reader.read(); if (done) break; if (value?.items) streamed.push(...value.items); }
          let textItems = streamed.filter((item: any) => typeof item.str === 'string' && item.str.trim()).map((item: any) => ({ ...item })) as TextItem[];
          if (textItems.length === 0) { const content = await page.getTextContent({ includeMarkedContent: false }); textItems = content.items.filter((item: any) => typeof item.str === 'string' && item.str.trim()).map((item: any) => ({ ...item })) as TextItem[]; }
          if (textItems.length > 0) {
            const content = await page.getTextContent({ includeMarkedContent: false });
            const styles = content.styles ?? {};
            if (!cancelled) setItems(textItems.map((item) => ({ ...item, style: styles[item.fontName ?? ''] })));
            return;
          }
          const canvas = rootRef.current?.parentElement?.querySelector('canvas') ?? null;
          if (canvas && !cancelled) {
            setSource('ocr');
            const worker = await createWorker('eng');
            try {
              const result = await worker.recognize(canvas); const data: any = result.data; const dpr = Math.max(1, canvas.width / Math.max(1, viewport.width));
              const words: OcrWord[] = (data.words ?? []).map((word: any) => ({ text: String(word.text ?? '').trim(), x: Number(word.bbox?.x0 ?? 0) / dpr, y: Number(word.bbox?.y0 ?? 0) / dpr, width: Math.max(0, Number(word.bbox?.x1 ?? 0) - Number(word.bbox?.x0 ?? 0)) / dpr, height: Math.max(0, Number(word.bbox?.y1 ?? 0) - Number(word.bbox?.y0 ?? 0)) / dpr, confidence: Number(word.confidence ?? 0) })).filter((word: OcrWord) => word.text && word.width > 1 && word.height > 1);
              if (!cancelled) setOcrWords(words);
            } finally { await worker.terminate(); }
            return;
          }
        } catch (error) { if (attempt === 7) console.error('[PDF Editor] text/OCR layer failed:', error); }
        await new Promise((resolve) => window.setTimeout(resolve, 150));
      }
      if (!cancelled) { setItems([]); setOcrWords([]); }
    };
    void loadText();
    return () => { cancelled = true; };
  }, [page, enabled, viewport]);

  useEffect(() => {
    if (editingIndex === null || !inputRef.current) return;
    const block = blocks[editingIndex]; if (!block) return;
    setEditFontSize(block.fontSize); setEditFontFamily(supportedFamily(block.fontFamily)); setEditFontWeight(block.fontWeight); setEditFontStyle(block.fontStyle); committedEditRef.current = false;
    inputRef.current.focus();
    const range = document.createRange(); range.selectNodeContents(inputRef.current); const selection = window.getSelection(); selection?.removeAllRanges(); selection?.addRange(range);
  ...
  return <div ref={rootRef} aria-label="PDF text layer" data-text-item-count={source === 'ocr' ? ocrWords.length : items.length} data-text-block-count={blocks.length} data-text-source={source} style={{ position: 'absolute', inset: 0, overflow: 'visible', pointerEvents: 'auto' }}>
    {blocks.map((block, index) => {
      ...
      const commitEdit = (value: string) => {
        ...
        if (replacement !== block.text || activeFontSize !== block.fontSize || activeFamily !== supportedFamily(block.fontFamily) || activeWeight !== block.fontWeight || activeStyle !== block.fontStyle) { committedEditRef.current = true; onEdit(selection, replacement); }
        setEditingIndex(null);
      };
      if (editing) return <React.Fragment key={`block-${index}`}>
        <div ref={toolbarRef} role="toolbar" aria-label="PDF text formatting" onMouseDown={(event) => event.preventDefault()} style={{ position: 'absolute', left: block.x, top: Math.max(2, block.y - 38), display: 'flex', alignItems: 'center', gap: 4, padding: 4, background: '#fff', border: '1px solid #d0d5dd', borderRadius: 6, boxShadow: '0 6px 18px rgba(16,24,40,.16)', zIndex: 20, whiteSpace: 'nowrap' }}>
          <select aria-label="Font" value={activeFamily} onChange={(event) => setEditFontFamily(event.target.value)} style={{ ...toolbarInput, width: 130 }}><option value="Arial, Helvetica, sans-serif">Arial / Helvetica</option><option value="Times New Roman, serif">Times New Roman</option><option value="Courier New, monospace">Courier New</option></select>
