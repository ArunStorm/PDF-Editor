...
describe('pdfTools', () => {
  it('merges PDFs', async () => {
    const a = await samplePdf(2);
    const b = await samplePdf(1);
  ...
  it('splits selected pages', async () => {
    ...
    const result = await splitPdf(source, [{ start: 2, end: 3 }]);
  ...
  it('rotates a page and updates metadata', async () => {
    ...
    const rotated = await rotatePage(source, 1, 90);
    const withMetadata = await updateMetadata(rotated, { title: 'PDF Editor Test', author: 'PDF Editor' });
    const doc = await PDFDocument.load(withMetadata);
    expect(doc.getPage(0).getRotation().angle).toBe(90);
    expect(doc.getTitle()).toBe('PDF Editor Test');
  });
