...
test('resume-like sample does not render word-level blue boxes in Edit Text mode', async ({ page }) => {
  await page.goto('/');
  ...
  expect(targetCount).toBe(blockCount);
  for (let i = 0; i < Math.min(targetCount, 25); i += 1) {
    const style = await targets.nth(i).evaluate((element) => { const computed = getComputedStyle(element); return { border: computed.border, outline: computed.outline, background: computed.backgroundColor, boxShadow: computed.boxShadow }; });
    expect(style.border).toMatch(/0px|transparent/);
    expect(style.outline).toMatch(/none|0px/);
    expect(style.background).toMatch(/rgba\(0, 0, 0, 0\)|transparent/);
    expect(style.boxShadow).toMatch(/none/);
  }
});
